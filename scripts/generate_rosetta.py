...
for i in range(len(decoys)):
    if(decoys[i].endswith('.pdb')):
        ...
        p=Pose()
        pose_from_file(p, decoy_dir + '/' + decoys[i])
        starting_p = Pose()
        starting_p.assign(p)
        dump_pdb(p, output_path + '/' + decoys[i])

decoys = os.listdir(output_path)

for i in range(len(decoys)):
        if(decoys[i].endswith('.pdb')):
            dec_res_list=[]
            dec_res_no = []
            with open(output_path + "/" + decoys[i]) as dFile:
                    for line in dFile: 
                            if(line[0:(0+4)]=="ATOM"):
            ...
            if(len(dec_res_list) > 0):
                    ...
                    residueList=[]        
                    start_end_ResNo = []    
                    with open(output_path + '/' + decoys[i]) as file:        
                            for line in file:                                         
                                    if(line[0:(0+4)]=="ATOM"):                                                       
                    ...
                    try:
                            pose = Pose()
                            pdb = Pose()
                            pose_from_file(pdb, output_path + '/' + decoys[i])
                            ...
                            #write output#
                            outFile = open(output_path + '/' + decoys[i].split('.pdb')[0] +".rosetta", "w")
